...
void BoardHistory::makeBoardMoveAssumeLegal(Board& board, Loc moveLoc, Player movePla) {
  ...
  Color maybeWinner = GameLogic::checkWinnerAfterPlayed(board, *this, movePla, moveLoc, bufferForCheckingWinner);
...
Hash128 BoardHistory::getSituationRulesHash(const Board& board, const BoardHistory& hist, Player nextPlayer) {
 //Note that board.pos_hash also incorporates the size of the board.
  ...
  hash ^= Hash128(
    Hash::murmurMix(Rules::ZOBRIST_MAXMOVES_HASH_BASE.hash0 ^ hist.rules.maxMoves),
    Hash::splitMix64(Rules::ZOBRIST_MAXMOVES_HASH_BASE.hash1 ^ hist.rules.maxMoves));
