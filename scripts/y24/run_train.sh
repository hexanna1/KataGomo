...
TRAINING_NAME="${TRAINING_NAME:-y24-main}"
...
DEFAULT_SEED_NAME="y21-main-latest"
DEFAULT_SEED_MODEL="../y21/data/latest.bin.gz"
DEFAULT_INITIAL_CHECKPOINT="../y21/data/train/y21-main/checkpoint.ckpt"
if [ "${SEED_MODEL+x}" != "x" ]
then
...
do
    ...
    (
        ...
        POS_LEN=24 CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" TRAIN_DEVICE="$TRAIN_DEVICE" bash train.sh "${TRAIN_ARGS[@]}"
        CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" bash export.sh y24 "$DATA_DIR_ABS" 0
    )
done
