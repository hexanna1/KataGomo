...
MODEL_KIND="${MODEL_KIND:-b10c256nbt-fson-rvgl-bnh}"
TRAINING_NAME="${TRAINING_NAME:-oy21-main}"
...
DEFAULT_SEED_NAME="oy17-main-latest"
DEFAULT_SEED_MODEL="../oy17/data/latest.bin.gz"
DEFAULT_INITIAL_CHECKPOINT="../oy17/data/train/oy17-main/checkpoint.ckpt"
if [ "${SEED_MODEL+x}" != "x" ]
then
...
do
    ...
    (
        ...
        POS_LEN=21 BOARD_SHAPE=obtuseY CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" TRAIN_DEVICE="$TRAIN_DEVICE" bash train.sh "${TRAIN_ARGS[@]}"
        CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" bash export.sh oy21 "$DATA_DIR_ABS" 0
    )
done
