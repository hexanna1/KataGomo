...
TRAINING_NAME="${TRAINING_NAME:-by5-main}"
...
SEED_MODEL_NAME="${SEED_MODEL_NAME:-seed}"
...
do
    ...
    (
        ...
        POS_LEN=7 BOARD_SHAPE=bentY CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" TRAIN_DEVICE="$TRAIN_DEVICE" bash train.sh "${TRAIN_ARGS[@]}"
        CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" bash export.sh by5 "$DATA_DIR_ABS" 0
    )
done
