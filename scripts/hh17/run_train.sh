...
TRAINING_NAME="${TRAINING_NAME:-hh17}"
...
DEFAULT_SEED_NAME="hh11-latest"
DEFAULT_SEED_MODEL="../hh11/data/latest.bin.gz"
DEFAULT_INITIAL_CHECKPOINT="../hh11/data/train/hh11/checkpoint.ckpt"
if [ "${SEED_MODEL+x}" != "x" ]
then
...
do
    ...
    (
        ...
        CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" PYTHON="$PYTHON" bash export.sh hh17 "$DATA_DIR_ABS" 0
    )
done
