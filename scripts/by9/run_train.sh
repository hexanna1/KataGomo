...
TRAINING_NAME="${TRAINING_NAME:-by9-main}"
...
DEFAULT_SEED_NAME="by5-latest"
DEFAULT_SEED_MODEL="../by5/data/latest.bin.gz"
DEFAULT_INITIAL_CHECKPOINT="../by5/data/train/by5-main/checkpoint.ckpt"
if [ "${SEED_MODEL+x}" != "x" ]
then
...
then
    if [ ! -f "$SEED_MODEL" ]
    then
        echo "Seed model not found: $SEED_MODEL"
        exit 1
    fi
    if [ ! -f "$DATA_DIR"/models/"$SEED_MODEL_NAME"/model.bin.gz ]
    then
...
do
    ...
    then
        ...
        echo "Set ENGINE=/path/to/katago, or build ../../build-opencl/katago."
        exit 1
    fi
    ...
    (
        ...
        then
            bash shuffle.sh "$DATA_DIR_ABS" "$TMP_DIR_ABS" "$SHUFFLE_THREADS" "$BATCH_SIZE" \
        ...
        POS_LEN=13 BOARD_SHAPE=bentY CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" TRAIN_DEVICE="$TRAIN_DEVICE" bash train.sh "${TRAIN_ARGS[@]}"
        CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" bash export.sh by9 "$DATA_DIR_ABS" 0
    )
done
