...
TRAINING_NAME="${TRAINING_NAME:-y21-main}"
...
do
    ...
    (
        ...
        POS_LEN=21 CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" TRAIN_DEVICE="$TRAIN_DEVICE" bash train.sh "${TRAIN_ARGS[@]}"
        CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" bash export.sh y21 "$DATA_DIR_ABS" 0
    )
done
