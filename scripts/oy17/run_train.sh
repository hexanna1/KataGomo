...
TRAINING_NAME="${TRAINING_NAME:-oy17-main}"
...
do
    ...
    (
        ...
        POS_LEN=17 BOARD_SHAPE=obtuseY CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" TRAIN_DEVICE="$TRAIN_DEVICE" bash train.sh "${TRAIN_ARGS[@]}"
        CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" bash export.sh oy17 "$DATA_DIR_ABS" 0
    )
done
