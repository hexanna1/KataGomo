...
TRAINING_NAME="${TRAINING_NAME:-b5nbt-15}"
...
DEFAULT_SEED_NAME="b5nbt-11-latest"
DEFAULT_SEED_MODEL="../b5nbt-11/data/latest.bin.gz"
DEFAULT_INITIAL_CHECKPOINT="../b5nbt-11/data/train/b5nbt-11/checkpoint.ckpt"
if [ "${SEED_MODEL+x}" != "x" ]
then
...
do
    ...
    (
        ...
        CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" PYTHON="$PYTHON" bash export.sh b5nbt-15 "$DATA_DIR_ABS" 0
    )
done
