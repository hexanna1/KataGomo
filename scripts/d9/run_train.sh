...
TRAINING_NAME="${TRAINING_NAME:-d9}"
...
DEFAULT_SEED_NAME="d6-latest"
DEFAULT_SEED_MODEL="../d6/data/latest.bin.gz"
DEFAULT_INITIAL_CHECKPOINT="../d6/data/train/d6/checkpoint.ckpt"
if [ "${SEED_MODEL+x}" != "x" ]
then
...
do
    ...
    (
        ...
        CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" PYTHON="$PYTHON" bash export.sh d9 "$DATA_DIR_ABS" 0
    )
done
