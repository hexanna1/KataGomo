...
class QuaxGUI:
    ...
    def play(self, x: int, row: int, diamond: bool, direction: str = "") -> None:
        if (x, row, diamond) in self.occupied():
            return
        self.moves.append(Move(self.current_player(), x, row, diamond, direction))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", type=Path, default=script_dir / "s9/gtp.local.cfg")
    parser.add_argument("--model", type=Path, default=script_dir / "s9/data/latest.bin.gz")
