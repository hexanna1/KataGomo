...
class Move:
    ...
    def token(self) -> str:
        if self.x is None or self.row is None:
            return "pass"
        return f"{column_letters(self.x)}{self.row + 1}{'*' if self.diamond else ''}"
...
class QuaxGUI:
    def __init__(self, root: tk.Tk, args: argparse.Namespace):
        ...
        self.analysis_by_cell: dict[tuple[int, int, bool], AnalysisMove] = {}
    ...
    def play(self, x: int, row: int, diamond: bool) -> None:
        if (x, row, diamond) in self.occupied():
            return
        self.moves.append(Move(self.current_player(), x, row, diamond))
    ...
    def top_ranked_move(self) -> Optional[tuple[int, int, bool]]:
        ...
        for key, analysis in self.analysis_by_cell.items():
            ...
            rank = (order, -visits, key[1], key[0], key[2])
        ...
        if self.engine is not None:
            ...
            if self.analysis_enabled:
                self.analysis_by_cell = {
                    (item.x, item.row, item.diamond): item
    ...
    def analysis_fill(
        ...
        analysis = self.analysis_by_cell.get(key)
        if analysis is None:
    ...
    def redraw(self) -> None:
        ...
        diamond_analysis_font = tkfont.Font(
            family="Helvetica", size=max(6, int(diamond_radius * 0.32)), weight="bold",
        )
        ...
        for row in range(self.size - 1):
            for x in range(self.size - 1):
                ...
                for a, b in [((x, row), (x + 1, row + 1)), ((x + 1, row), (x, row + 1))]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Quax board and KataGomo analysis GUI")
    parser.add_argument("--size", type=int, default=9)
    parser.add_argument("--engine", type=Path, default=default_engine_path(script_dir))
    parser.add_argument("--config", type=Path, default=script_dir / "d9/gtp.local.cfg")
    parser.add_argument("--model", type=Path, default=script_dir / "d9/data/latest.bin.gz")
